...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        z = X.dot(self.w.T)
        prob = LogisticRegression.get_sigmoid_value(z)
        y_proba = np.vstack((1.0-prob, prob)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        z = X_batch.dot(self.w.T)
        h = LogisticRegression.get_sigmoid_value(z)
        
        loss = -(y_batch.dot(np.log(h)) + (1.0-y_batch).dot(np.log(1.0-h)))
        dw = X_batch.T * (h - y_batch)
        ...
        loss += 0.5 * reg * np.dot(self.w[:-1], self.w[:-1]) / num_train
        dw[:-1] += (reg * self.w[:-1]) / num_train

        return loss, dw
    
    @staticmethod
    def get_sigmoid_value(z):
